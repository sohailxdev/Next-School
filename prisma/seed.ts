...
async function main() {
  ...
  const adminUser = await createClerkUser({
    username: "admin",
  ...
  // Teacher
  const teacherUser = await createClerkUser({
    username: "teacher",
  ...
  await prisma.teacher.create({
    data: {
      ...
      name: teacherUser.firstName ?? "Teacher",
      surname: teacherUser.lastName ?? "User",
      email:
        teacherUser.emailAddresses[0]?.emailAddress ?? "teacher@example.com",
      phone: "123-456-7890",
  ...
  // Student
  const studentUser = await createClerkUser({
    username: "student",
  ...
  await prisma.student.create({
    data: {
      id: studentUser.id,
      username: studentUser.username ?? "student",
      name: studentUser.firstName ?? "Student",
      surname: studentUser.lastName ?? "User",
      email:
        studentUser.emailAddresses[0]?.emailAddress ?? "student@example.com",
      phone: "987-654-3210",
      address: "Student Address",
      bloodType: "O-",
      sex: UserSex.FEMALE,
      parent: { connect: { id: "parentId1" } }, // Connect to an existing parent
      grade: { connect: { id: 1 } }, // connect grade relation instead of gradeId
      class: { connect: { id: 1 } },
